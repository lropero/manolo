...
class Tournament {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (
          ...
          throw new Error(
            'Tournament requires config.levels to be properly configured'
          )
        }
        if (playersPerTable < 2 || playersPerTable > 9) {
          throw new Error(
            'Tournament requires config.playersPerTable to be between 2 and 9'
          )
        }
        if (!(startingChips > 0)) {
          throw new Error(
            'Tournament requires config.startingChips to be greater than 0'
          )
        }
        ...
        for (const [index, playerName] of playerNames.entries()) {
          if (!isValidPlayerName({ playerName })) {
            throw new Error(
              `Player ${index + 1} (${playerName}) has invalid player name`
            )
          }
          if (
            playerNames.reduce(
              (count, name) => count + (name === playerName),
              0
            ) > 1
          ) {
        ...
        const players = playerNames.map(
          playerName => new Player({ chips: startingChips, name: playerName })
        )
  ...
  async play () {
    ...
    while (
      !pause &&
      (this.tables.length > 1 || this.tables[0].players.length > 1)
    ) {
      ...
      if (
        ...
        this.config.levels
          ...
          .reduce((limit, level) => limit + level[3], 0)
      ) {
        this.handCountPerLevel = 0
        this.level++
      }
      ...
      for (const table of this.tables) {
        ...
        eliminated.push(
          ...table.players
            .filter(player => player.isBroke())
            .map(player => [table.id, player.name])
        )
  ...
  async reaccommodateTables () {
    if (this.tables.length > 1) {
      for (const tableWithOnePlayer of this.tables.filter(
        table => table.players.length === 1
      )) {
        this.playersWaitingTable.push(tableWithOnePlayer.players[0])
        this.tables = this.tables.filter(
          table => table.id !== tableWithOnePlayer.id
        )
      }
      let freeSeats =
        this.tables.length * this.config.playersPerTable -
        [].concat(
          ...this.tables.map(table => table.players),
          this.playersWaitingTable
        ).length
      while (
        freeSeats > this.config.playersPerTable &&
        ...
        for (const player of smallestTable.players.concat(
          this.playersWaitingTable
        )) {
          const restOfTablesWithOpenSeats = this.tables.filter(
            table =>
              table.id !== smallestTable.id &&
              table.players.length < this.config.playersPerTable
          )
          const table =
            restOfTablesWithOpenSeats[
              Math.floor(Math.random() * restOfTablesWithOpenSeats.length)
            ]
          if (table) {
            table.addPlayer({ player })
          } else if (
            !this.playersWaitingTable.find(p => p.name === player.name)
          ) {
        ...
        const seated = []
          .concat(...this.tables.map(table => table.players))
          .map(player => player.name)
        this.playersWaitingTable = this.playersWaitingTable.filter(
          player => !seated.includes(player.name)
        )
        freeSeats =
          this.tables.length * this.config.playersPerTable -
          [].concat(
    ...
    if (this.playersWaitingTable.length) {
      let tableWithFreeSeat = this.tables.find(
        table => table.players.length < this.config.playersPerTable
      )
      while (tableWithFreeSeat && this.playersWaitingTable.length) {
        tableWithFreeSeat.addPlayer({ player: this.playersWaitingTable.pop() })
        tableWithFreeSeat = this.tables.find(
          table => table.players.length < this.config.playersPerTable
        )
      }
      if (
        this.playersWaitingTable.length >
        Math.floor(this.config.playersPerTable / 2)
      ) {
  ...
  async run () {
    const winner = await this.play()
    this.logger(
      gradient.rainbow(`Winner is ${winner.name} with ${winner.stack} chips`)
    )
  }
  ...
  seatPlayers ({ players }) {
    ...
    if (
      ...
      if (temp2.length === 1) {
        if (
          !this.playersWaitingTable.find(
            player => player.name === temp2[0].name
          )
        ) {
        ...
        new Table({
          id:
            ((this.tables[this.tables.length - 1] &&
              this.tables[this.tables.length - 1].id) ||
              0) + 1,
          players: group,
          tournament: this
        })
